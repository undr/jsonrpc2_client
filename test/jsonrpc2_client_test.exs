defmodule JSONRPC2ClientTest do
  ...
  describe ".send" do
    test "invocation request", %{bypass: bypass} do
      ...
      assert {:ok, %Response.Result{id: 1, value: "value"}} == JSONRPC2Client.send(req, "http://localhost:51346/jsonrpc")
    end

    test "notification request", %{bypass: bypass} do
      Bypass.expect bypass, "POST", "/jsonrpc", fn conn ->
        {:ok, body, conn} = Plug.Conn.read_body(conn)
        assert body == ~s<{"jsonrpc":"2.0","method":"method","params":[1,2,3]}>
        assert Plug.Conn.get_req_header(conn, "content-type") == ["application/json"]
      ...
      assert {:ok, []} == JSONRPC2Client.send(req, "http://localhost:51346/jsonrpc")
    end
  end
end
